module ImprovedHagerZhangLinesearch

using ManifoldsBase
using ManifoldsBase: allocate_result

using Manopt
using Manopt: max_stepsize, get_cost_function
    ...
    while iter < linesearchmax
        ...
        if B - A < ls.gamma * (b - a)
            if nextfloat(values[ia]) >= values[ib] && nextfloat(values[iA]) >= values[iB]
                ...
                ls.mayterminate = false # reset in case another initial guess is used next
                # see https://github.com/JuliaNLSolvers/LineSearches.jl/issues/173
                if values[ib] < values[iA]
                    return b, values[ib]
                else
                    return A, values[iA]
                end
